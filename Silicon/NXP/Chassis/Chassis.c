  ...
  )
{
  ...
  Ver = SVR_SOC_VER (Svr);
  AsciiStrCpyS (Buf, 6, "SOC: ");

  for (Count = 0; Count < ARRAY_SIZE (CpuTypeList); Count++) {
    if ((CpuTypeList[Count].SocVer & SVR_WO_E) == Ver) {
      AsciiStrCatS (Buf, AsciiStrLen((CONST CHAR8 *)CpuTypeList[Count].Name), (CONST CHAR8 *)CpuTypeList[Count].Name);

      if (Ver == (SVR_LX2160A & SVR_WO_E)) {
        if (!((Svr >> 12) & 0x1)) {
          AsciiStrCatS (Buf, 2, (CONST CHAR8 *)"C");
        }
      }

      if (IS_E_PROCESSOR (Svr)) {
        AsciiStrCatS (Buf, 2, (CONST CHAR8 *)"E");
      }
  ...
  if (Count == ARRAY_SIZE (CpuTypeList)) {
    AsciiStrCatS (Buf, 8, (CONST CHAR8 *)"unknown");
  }
